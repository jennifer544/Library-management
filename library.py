...
def register():
    while True:
        try:
            print("\033[1mRegister your details here.\033[0m")
    ...
    try:
        ...
        if result:
            ...
            print(tabulate(result,
                           headers=["Book ID", "Title", "Author ID", "Genre", "Description", "Status", "Created At"]))
        else:
            print("No books found.")
            return

        # Prompt user to enter the book title to borrow
        book_title = input("\nEnter book title to rent: ")
        ...
        cursor.execute("UPDATE books SET status='Rented' WHERE book_id=%s", (book_id,))
        databaseobj.commit()
        print(f"Book '{book_title}' has been rented successfully. Due date is {due_date.strftime('%Y-%m-%d')}.")
...
def update_membership_plan():
    try:
        # Display all membership plans before updating
        cursor.execute("SELECT * FROM Plans ORDER BY membership_id ASC")
        plans = cursor.fetchall()
        if plans:
            print("\nAvailable membership plans:")
            print(tabulate(plans, headers=["Membership ID", "Membership Plan", "Price"]))
        else:
            print("No membership plans found.")
            return

        # Prompt admin to enter the membership ID to update
        membership_id = input("\nEnter the Membership ID to update: ")

        # Check if the membership plan exists
        cursor.execute("SELECT * FROM Plans WHERE membership_id=%s", (membership_id,))
        plan = cursor.fetchone()
        if not plan:
            print("Membership plan not found.")
            return

        # Input new details for the membership plan
        new_plan_name = input("Enter new plan name (leave blank to keep current): ")
        new_price = input("Enter new price (leave blank to keep current): ")

        # Update the membership plan in the database
        update_query = "UPDATE Plans SET "
        update_data = []
        if new_plan_name:
            update_query += "membership_plan = %s"
            update_data.append(new_plan_name)
        if new_price:
            if update_data:
                update_query += ", "
        ...
        update_data.append(membership_id)
...
def view_feedback():
    try:
        # Exclude usernames that start with 'admin'
        cursor.execute("""
            SELECT f.feedback_id, u.username, f.content, f.date_submitted 
            FROM feedback f 
...
def list_of_users():
    query = """
    SELECT user_id,username, email from users where username not like "%admin%"
    """
    cursor.execute(query)
    query_result = cursor.fetchall()
    if query_result:
        print(tabulate(query_result, headers=["user_id","username", "email_id"]))
    else:
...
def add_membership_plan():
    try:
        # Input new membership plan details
        plan_name = input("Enter the membership plan name: ")
        price = input("Enter the price for the membership plan: ")
        ...
        query = "INSERT INTO Plans (membership_plan, price) VALUES (%s, %s)"
        cursor.execute(query, (plan_name, price))
        databaseobj.commit()
...
def admin_panel():
    while True:
        print("\033[1m\033[3m\033[4mAdmin Panel\033[0m")
        print("1. View Users")
        print("2. Add Author")
        ...
        print("5. List Books")
        print("6. View Genres")
        print("7. View Author Information")
        print("8. View Feedback")  # New option to view feedback
...
def member_panel():
    while True:
        print("\033[1m\033[3m\033[4mMember Panel\033[0m")
        print("1. View Membership Plans")
        print("2. View Books")
        print("3. View Genres")
        print("4. View Author Information")
        print("5. Rent A Book")
        print("6.Update Membership Plan")
...
def login():
    ...
    if user:
        ...
        print(f"\033[1m\033[3mWelcome {username}!\033[0m")
...
def main():
    while True:
        ...
        print("\033[1m\033[3m\033[4mLibrary Management System\033[0m")
        print("1. Register")
        print("2. Login")
        print("3. Exit")
